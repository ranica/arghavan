...
const getters = {
     ...
     */
    allRecords: state => state._all_province,
...
const mutations = {
     ...
     * set all Provinces
     */
    setAllProvinces: (state, data) => state._all_province = data,
    /**
     * Insert a Record
     */
    ...
    updateRecords(context, data) {
        return new Promise((resolve, reject) => {
            context.dispatch('updateModule/updateRecords', data)
                .then(res => {
                    let updatedRecord = res.data.province;
    ...
    createRecords(context, data) {
        return new Promise((resolve, reject) => {
            context.dispatch('createModule/createRecords', data)
                .then(res => {
                    if (res.data.status == 0) {
                        context.commit('createRecord', res.data.province);
                    }
