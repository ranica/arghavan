...
{	
	...
	loadRecords(context, page)
	{
		...
		{
			axios.get('/situations?page=' + page)
				.then(res =>
				{
	...
	saveRecord: (context, record) =>
	{
		return new Promise((resolve, reject) =>
		{
			...
			if (0 == record.id)
			{
				...
				axios.post('/situations', record)
					.then(res => {
						...
						let newRecord = res.data.situation;
			...
			{
				axios.put('/situations/' + record.id, record)
					.then(res => {
						let status       = (0 == res.data.status);
						let updatedRecord = res.data.situation;
	...
	deleteRecord(context, id)
	{
		return new Promise((resolve, reject) =>
		{
			...
			// Try to delete
			axios.delete('/situations/' + id)
				.then(res =>
				{
					if (0 == res.data.status)
					{
						// Remove form situations list
