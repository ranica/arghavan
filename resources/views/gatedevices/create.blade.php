	...
	<div class="card">
		...
		<div class="card-content f-BYekan">
			...
			<form>
				...
				<div class="form-group label-floating"  :class="{'has-error' : errors.has('gategender_id')}">
					<label class="control-label">جنسیت تردد کننده</label>
					<select v-model="tempRecord.gategender.id" class="form-control" name="gategender_id" required>
				...
				<div class="form-group label-floating"  :class="{'has-error' : errors.has('gatepass_id')}">
					<label class="control-label">نحوه عبور از گیت</label>
					<select v-model="tempRecord.gatepass.id" class="form-control" name="gatepass_id" required>
						<option v-for="gatepass in gatepasses" :value="gatepass.id"
							:selected="tempRecord.gatepass.id == gatepass.id">
							@{{ gatepass.name }}
						</option>
					</select>
					<span class="material-input"></span>
				...
				<div class="form-group label-floating"  :class="{'has-error' : errors.has('gatedirect_id')}">
					...
					<select v-model="tempRecord.gatedirect.id" class="form-control" name="gatedirect_id" required>
				...
				<div class="form-group label-floating"  :class="{'has-error' : errors.has('zone_id')}">
					<label class="control-label">ناحیه تردد</label>
					<select v-model="tempRecord.zone.id" class="form-control" name="zone_id" required>
						<option v-for="zone in zones" :value="zone.id"
							:selected="tempRecord.zone.id == zone.id">
							@{{ zone.name }}
						</option>
					</select>
					<span class="material-input"></span>
				...
				<div class="form-group label-floating" :class="{'has-error' : errors.has('timepass')}">
					<label class="control-label">زمان عبور از گیت </label>
					<input class="form-control" type="text"  name="timepass" minlength="1" maxlength="50" autofocus required
						v-validate="'required|min:1|max:50'" data-vv-delay="250" v-model="tempRecord.timepass"
						/>
					<i v-show="errors.has('timepass')" class="fa fa-warning"></i>
                    <span v-show="errors.has('timepass')" class="help is-danger">زمان عبور نامعتبر می باشد</span>
					<span class="material-input"></span>
				</div>

				<div class="form-group label-floating" :class="{'has-error' : errors.has('timeserver')}">
					<label class="control-label">زمان پاسخ سرور </label>
					<input class="form-control" type="text"  name="timeserver" minlength="1" maxlength="50" autofocus required
						v-validate="'required|min:1|max:50'" data-vv-delay="250" v-model="tempRecord.timeserver"
						/>
				...
				<div class = "input-group" :class="{'has-error' :errors.has('state')}">
                    <div class="togglebutton">
                        <label>
                            <input class="form-check-input" checked="" type="checkbox" name="state" id="state" v-model="tempRecord.state">
                            فعال
                        </label>
                    </div>
                </div>
